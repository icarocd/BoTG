/**
 * Implementacao de Stemming para a lingua inglesa, como proposto por Porter.
 * Acessado em http://tartarus.org/~martin/PorterStemmer/, em 16 de Novembro de 2008.
 */
...
public class PorterStemmer implements Stemmer
{
	...
	@Override
	public synchronized String getStem(String term) {
		add(term);
		stem();
		return getStem();
	}
	...
	private void expandIfNecessary(int ncharsToAdd)
	{
		if (i + ncharsToAdd >= b.length) {
			char[] new_b = new char[i + ncharsToAdd + INC];
			for (int c = 0; c < i; c++) {
                new_b[c] = b[c];
            }
			b = new_b;
	...
	/**
	 * After a word has been stemmed, it can be retrieved by getStem(), or a reference to the internal
	 * buffer can be retrieved by getResultBuffer and getResultLength (which is generally more efficient.)
	 */
	...
	@Override
	public boolean stemOnlyToValidWords() {
	    return false;
	}
